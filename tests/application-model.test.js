...
function fixture() {
  ...
  write(path.join(root, ".autotap/contracts/auth.contract.ts"), `import { defineContract } from "tapp-mcp/contracts";
...
test("a multi-target workspace uses the repository name instead of the first module name", async () => {
  ...
  write(path.join(root, "alpha/.autotap/contracts/auth.contract.ts"), `import { defineContract } from "tapp-mcp/contracts"; export default defineContract({name:"alphaAuthWorks",title:"Alpha authentication",businessValue:"Alpha users enter",criticality:"high",platforms:["android"],actors:{customer:{}},steps:[{actor:"customer",task:"signIn"}]});`);
...
test("application model merges explicit actors and contract placeholders without persisting values", async () => {
  ...
  write(path.join(root, ".autotap/contracts/social.contract.ts"), `import { defineContract } from "tapp-mcp/contracts";
