...
export function writeHtmlReport(captureDir, { report, label = "" } = {}) {
  ...
  const shotsHtml = shots
    .map((f) => {
      const caption = f.replace(/^state_\d+_/, "").replace(/\.png$/, "").replace(/[-_]/g, " ");
      return `<figure><a href="${esc(f)}"><img loading="lazy" src="${esc(f)}" alt="${esc(caption)}"></a><figcaption>${esc(caption)}</figcaption></figure>`;
    })
    .join("\n");
